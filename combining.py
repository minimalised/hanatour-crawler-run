        ...
        while retry_count < 3:
            try:
                response = await aclient.chat.completions.create(
                    ...
                    messages=[
                        {
                            ...
                            "content": (
                                ...
                                "2. 문장부호 및 특수문자 사용 제한:\n"
                                "   - 최종 상품명 내부에는 쉼표(,), 느낌표(!), 물결(~), 플러스(+) 같은 부호나 특수문자를 절대 포함할 수 없음.\n"
                                "   - 기간이나 일정 범위를 나타낼 때는 반드시 붙임표 대시 기호(예: '5-6일')로 치환하여 표현할 것.\n"
                                "3. 광고성 단어 및 시스템 지시어 배제:\n"
                                "   - '신상품', '특가', '폭탄세일', '땡처리' 같은 유치한 홍보성 단어는 전면 배제할 것.\n"
                                "   - 결과물에 '주의:', '경고:', '안내:' 등 시스템 지시어 성격의 텍스트를 삽입하는 것을 절대 금지함.\n"
                                "4. 네이밍 구성 요소:\n"
                                "   - 핵심 여행 지역, 일정, 주요 골프장/호텔 명사 위주의 실용적이고 직관적인 명사 위주로 변형 조합할 것.\n"
                                "5. 출력 형식:\n"
                                "   - 설명이나 서론, 후론 없이 오직 재구성된 상품명 '딱 한 줄'만 반환할 것.\n\n"
                                
                                "💡 [출력 예시 참고]\n"
                                "- 원본: [특가!!] 다낭 골프 투어 3박5일 ~ 빈펄 리조트 숙박 + 3대 골프장 라운딩!\n"
                                "- 추천 결과: 다낭 빈펄 리조트 골프 투어 3박5일 다낭CC 몽고메리 3대 라운드\n\n"
                                f"{extra_prompt}"
                            )
                        },
                    ...
                    max_tokens=80,
                    temperature=0.3 # 💡 규칙을 더 깐깐하게 지키도록 온도를 0.4에서 0.3으로 낮췄습니다.
                )
