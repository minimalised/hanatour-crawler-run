...
# ==========================================
# GitHub Secrets에 저장된 SPREADSHEET_KEY를 환경변수에서 가져옵니다.
SPREADSHEET_KEY = os.environ.get("SOURCE_SHEET_ID", "YOUR_SPREADSHEET_KEY_HERE") 
...
aclient = AsyncOpenAI(api_key=os.environ.get("OPENAI_API_KEY", "YOUR_API_KEY_HERE"))
...
# ==========================================
# 2. 데이터 로드 및 캐시 관리 (규칙 3 반영)
# ==========================================
def load_google_sheet_data():
    """구글 스프레드시트에서 데이터를 단 1번의 호출로 읽어옵니다."""
    scopes = ["https://www.googleapis.com/auth/spreadsheets"]
    
    # [변경] GitHub Secrets의 GOOGLE_JSON_RAW(문자열)를 파싱하여 인증에 사용합니다.
    ...
    processed_rows = []
    for idx, row in enumerate(rows, start=2): # 2번째 행부터 시작 (엑셀 기준)
        # G열(인덱스 6)까지 리스트 길이 보장 (IndexError 방지)
        while len(row) < 7:
            row.append("")
            
        p_id = row[0].strip()   # A열: 상품 고유 ID
        p_name = row[1].strip() # B열: 원본 상품명
        current_result = row[6].strip() # G열: 기존 재조합 결과물
...
# ==========================================
# 3. 비동기 LLM 호출 및 중복 검증 루프 (규칙 2, 5 반영)
# ==========================================
async def call_llm_with_retry(target: Dict, confirmed_pool: Set[str]) -> str:
    ...
    async with semaphore:
                    ...
                    messages=[
                        {
                            ...
                            "content": (
                                ...
                                "2. 브랜드명, 핵심 핵심 속성, 카테고리가 자연스럽게 연결되게 할 것\n"
        ...
        return f"{sanitized_name}_{target['id']}"
...
async def main():
    ...
    cleaned_cache = {p_id: data for p_id, data in old_cache.items() if p_id in current_product_ids}
